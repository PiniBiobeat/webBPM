...
class Test_me():
    ...
    def test_connect_to_db_in_lupa_DB(self):
        ...
        cursor = cnxn.cursor()
        print(cursor)
        cursor.execute("select *  FROM [lupa].[dbo].[orders_tbl] where bulk_id is null  and in_status = 'Printing process' and charged_date < DATEADD(hour, -24, GETDATE())")
    ...
    def test_connect_to_db_in_lupa_online_DB(self):
        ...
        cursor = cnxn.cursor()
        print(cursor)
        cursor.execute("select *  FROM [lupa_online].[dbo].[order_item_tbl] where bulk_id = 0   and in_status = 21 and charged_date < DATEADD(hour, -24, GETDATE())")
        rows = cursor.fetchall()
        if rows != []:
            for row in rows:
    ...
    def test_connect_to_db_in_lupa_tiles_DB(self):
        ...
        cursor = cnxn.cursor()
        print(cursor)
        cursor.execute("select *  FROM [lupa_square].[dbo].[order_item_tbl] where bulk_id = 0   and in_status = 21 and charged_date < DATEADD(hour, -24, GETDATE())")
        rows = cursor.fetchall()
        if rows != []:
            for row in rows:
                my_dict_lupa[row[1]] = row[2]
    ...
    def json_to_slack_message(self, json_data):
        ...
        value_list = "\n".join([f"• <https://admin.lupa.co.il/admin_book/Order_Details.aspx?id={key}|{key}>" if str(key).startswith('18') else f"• <https://admin.lupa.co.il/admin_online/Order_Details.aspx?id={key}|{key}>" if str(key).startswith('76') else f"• <https://admin.lupa.co.il/admin_tiles/Order_Details.aspx?id={key}|{key}>" for key in keys_list])
