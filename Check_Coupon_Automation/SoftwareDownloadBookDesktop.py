...
def send_hook():
    ...
    url = 'https://files.lupa.co.il/lp/hooks.aspx?method=old_coupon&campaign_name=SoftwareDownloadBookDesktop&messageid=6438390083944448&email=lupadevtest@gmail.com&discount=15&days=7'
...
def chack_if_email_exists(date_from_email_after_regex,subject,date_time_now):
    ...
    if subject == 'לך ורק לך! הטבה אישית ללופה בריבוע מחכה לך בפנים 🎁' and date_from_email_after_regex == date_time_now:
...
def send_email(subject, message):
    ...
    return requests.post(
        ...
        data={
              "from": "Lupa Automation ,Coupon Automation SoftwareDownloadBookDesktop  <monitor@lupa.co.il>",
              "to": operators,
              "subject": subject,
              "text": message,
              }
...
def if_email_not_exists_send_email(result):
    ...
    send_email("The email not sent ** SoftwareDownloadBookDesktop **", result)
...
def check_all_emails(first_email_id, latest_email_id,mail):
    ...
    for i in range(latest_email_id, first_email_id, -1):
        ...
        if isValid:
            break
    if not isValid:
         print("Email not exists --לך ורק לך! הטבה אישית ללופה בריבוע מחכה לך בפנים 🎁 -- ")
         if_email_not_exists_send_email("The user did not receive the email -- לך ורק לך! הטבה אישית ללופה בריבוע מחכה לך בפנים 🎁 --" + FROM_EMAIL)
