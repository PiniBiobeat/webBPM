...
def send_hook():
    ...
    url = 'https://files.lupa.co.il/lp/hooks.aspx?method=old_coupon&campaign_name=Birthday&messageid=5154647637557248&email=lupadevtest@gmail.com&discount=35&days=30'
...
def chack_if_email_exists(date_from_email_after_regex,subject,date_time_now):
    ...
    if subject == 'הפי יום הולדת עם מתנה מהחברים בלופה 🎁' and date_from_email_after_regex == date_time_now:
...
def send_email(subject, message):
    ...
    return requests.post(
        ...
        data={
              "from": "Lupa Automation ,Coupon Automation Birthday  <monitor@lupa.co.il>",
              "to": operators,
              "subject": subject,
              "text": message,
              }
...
def if_email_not_exists_send_email(result):
    ...
    send_email("The email not sent ** Birthday **", result)
...
def check_all_emails(first_email_id, latest_email_id,mail):
    ...
    for i in range(latest_email_id, first_email_id, -1):
        ...
        if isValid:
            break
    if not isValid:
         print("Email not exists --הפי יום הולדת עם מתנה מהחברים בלופה 🎁 -- ")
         if_email_not_exists_send_email("The user did not receive the email -- הפי יום הולדת עם מתנה מהחברים בלופה 🎁 --" + FROM_EMAIL)
