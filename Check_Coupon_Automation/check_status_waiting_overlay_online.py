...
def test_connect_to_db():
        ...
        cursor.execute(
                    '''  
                     ...
                     RIGHT   JOIN (select DATEADD(hour, -1 , GETDATE()) AS DATEADD  , order_id,master_id,in_status,charged_date,device FROM [lupa_online].[dbo].[orders_tbl] where in_status = 26) AS e ON o.order_id = e.order_id where DATEADD>e.charged_date and ISNULL(payload,'') = '' 
         ''')
        row = cursor.fetchall()
        for i in row:
            if i[2] == None:
